...
assert (
    "<p>DESCRIPTION: Flask Simple Example | Structure + Simplicity</p>"
    in request_root.text
)
assert "<p>VERSION: 0.0.1</p>" in request_root.text


request_math_add_sub = requests.get(
    "http://0.0.0.0:8080/add-sub?value_one=2&value_two=3"
)
...
request_math_add_sub = requests.get(
    "http://0.0.0.0:8080/mul-div?value_one=2&value_two=3"
)
